...
from models import User, PublicMessage, Group, GroupMember, SessionLocal
from ai_companion import get_mikasa_response
...
def list_friends():
        ...
        if friend_usernames:
            ...
            for fname in friend_usernames:
                ...
                if not user_obj:
                    ...
                    is_expired = True
                elif user_obj.expires_at and datetime.now(timezone.utc) > user_obj.expires_at.replace(tzinfo=timezone.utc):
                    is_expired = True
    ...
    # Inject MIKASA (AI Companion)
    friends_list.append({
        "username": "MIKASA",
        "online": True,
        "expired": False,
        "is_ai": True,  # Frontend flag
        "status": "AI Companion"
    })
...
def create_group():
    ...
    g_type = (data.get("type") or "public").strip()
    ...
    try:
        ...
        for m in memberships:
            ...
            result.append({
                ...
                "type": g.type,
                "role": m.role,
                "unread_count": 0 # Placeholder for now, could use Redis
            })
...
def search_groups():
    ...
    db = get_db()
    try:
         # Find public groups not joined yet
         user = db.query(User).filter_by(username=username).first()
         my_group_ids = [m.group_id for m in user.group_member_rel] if hasattr(user, 'group_member_rel') else [] 
         # Note: We didn't add backref on User side explicitly, let's query directly
         
         my_memberships = db.query(GroupMember).filter_by(user_id=user.id).all()
         my_group_ids = [m.group_id for m in my_memberships]
         
         groups = db.query(Group).filter(
             Group.name.ilike(f"%{query}%"),
             Group.type == "public",
             ~Group.id.in_(my_group_ids) if my_group_ids else True
         ).limit(10).all()
...
def join_group():
    ...
    try:
        ...
        if group.type == "private":
             return jsonify({"error": "Cannot join private group directly"}), 403
             
        # Check existing
        existing = db.query(GroupMember).filter_by(user_id=user.id, group_id=group.id).first()
        if existing:
            return jsonify({"success": True, "message": "Already joined"})
...
def get_group_members(group_id):
    ...
    try:
        ...
        user = db.query(User).filter_by(username=username).first()
        is_member = db.query(GroupMember).filter_by(user_id=user.id, group_id=group_id).first()
        if not is_member:
        ...
        for m in members:
            # Check online status
            is_online = redis_client.exists(f"online:{m.user.username}")
            result.append({
...
def handle_start_chat(data):
    ...
    if group_id:
        ...
        try:
             ...
             join_room(room) # Ensure joined (redundancy)
...
def handle_send_message(data):
    ...
    # Security check for Groups
    if room.startswith("group_"):
        gid = room.split("_")[1]
        # Ideally verify membership here again via DB or Redis cache
        # For MVP/Perf we trust the room logic if session is valid, 
        # but strictest is to check DB. Let's do a lightweight DB check or skip for speed.
        # User is in room only if authorized in connect/join.
        pass
    ...
    try:
        ...
        # --- AI TRIGGER ---
        if "MIKASA" in room:
             # Assume format user:MIKASA or MIKASA:user
             # Check if we should respond (only if message is FROM user)
             if username != "MIKASA":
                 # 1. Get History (Last 10 messages for context)
                 # Note: get_chat_messages fetches from Redis and decodes
                 # It returns dicts. We need to convert to format expected by AI.
                 history_msgs = get_chat_messages(room)
                 history = []
                 for m in history_msgs[-10:]:
                     role = "user" if m["from"] != "MIKASA" else "model"
                     history.append({"role": role, "text": m["message"]})
                     
                 # 2. Generate Response
                 ai_text = get_mikasa_response(message_text, history)
                 
                 # 3. Create & Save AI Message
                 ai_msg_id = secrets.token_hex(8)
                 ai_now = datetime.now(timezone.utc)
                 ai_expires_at = ai_now + timedelta(seconds=expiration_seconds)
                 ...
                 ai_data = {
                     ...
                     "expiration": expiration_seconds,
                     "expires_at": ai_expires_at.isoformat(),
                     "time_remaining": expiration_seconds,
                     "file_data": None
                 }
                 
                 # Save AI Response
                 r_key = f"message:{room}:{ai_msg_id}"
                 redis_client.setex(r_key, expiration_seconds, json.dumps(ai_data))
                 redis_client.rpush(chat_key, ai_msg_id)
                 
                 # Emit AI Response
                 socketio.emit("new_message", ai_data, room=room)
